...
export const createMenuItem = async (req, res) => {
    try {
        ...
        const menu = await Menu.create({ name, description, price, image, catId, is_publish })
        if (!menu) {
...
export const updateMenuInfo = async (req, res) => {
    try {
        ...
        const { name, description, price, image, catId } = req.body;
        if (!name || !price || !catId) {
            return res.status(400).json({ message: "name, price and category are required" })
        }
        if (typeof price !== "number" || price <= 0) {
            return res.status(400).json({ message: "price should be a valid positive number" })
        }
